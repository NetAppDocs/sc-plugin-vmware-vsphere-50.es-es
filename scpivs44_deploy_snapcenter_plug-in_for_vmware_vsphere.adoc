...
+
Puede que la puesta en marcha tarde unos minutos en completarse. Se indica una puesta en marcha correcta cuando el complemento de VMware de SnapCenter está encendido, las herramientas de VMware están instaladas y la pantalla le solicita que inicie sesión en el complemento de VMware de SnapCenter. Puede cambiar la configuración de red de DHCP a estática durante el primer reinicio. Sin embargo, no se admite el cambio de static a DHCP.
...
+
Si el complemento de VMware de SnapCenter no está habilitado, consulte link:scpivs44_restart_the_vmware_vsphere_web_client_service.html["Reinicie el servicio del cliente de VMware vSphere"].
