...
| Sistema de almacenamiento | Introduzca el FQDN o la dirección IP de Management LIF de un clúster de almacenamiento o una máquina virtual de almacenamiento. El complemento de VMware de SnapCenter no admite varias máquinas virtuales de almacenamiento con el mismo nombre en clústeres diferentes. 
...
| IP preferida | Si la máquina virtual de almacenamiento tiene más de una dirección IP de gestión, marque esta casilla e introduzca la dirección IP que desee usar el plugin de SnapCenter VMware. *Nota:* no utilice corchetes ([]) al introducir la dirección IP. 
